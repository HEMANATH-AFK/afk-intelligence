...
class GraphExtractor:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(
                    ...
                    if isinstance(node, ast.ClassDef):
                        for base in node.bases:
                            ...
                            if base_name:
                                self.graph.add_edge(
                                    node_id, base_name, relationship="inherits"
                                )
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        self.graph.add_edge(
                            rel_path, alias.name, relationship="imports"
                        )
                ...
                if isinstance(node, ast.Call):
                    if isinstance(node.func, ast.Name):
                        ...
                        self.graph.add_edge(
                            rel_path, node.func.id, relationship="calls"
                        )
    ...
    def get_neighbors(self, node_id: str, relationship: Optional[str] = None):
        ...
        for neighbor in self.graph.neighbors(node_id):
            if relationship:
                if (
                    self.graph.get_edge_data(node_id, neighbor).get("relationship")
                    == relationship
                ):
